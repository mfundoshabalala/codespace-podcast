...
class Component extends LitElement {
	...
	constructor () {
		...
		this.disconnectStore = connect((state) => {
			...
			if (this.favorite !== state.favorite) { this.favorite = state.favorite; }
	...
	render () {
		...
		const filteredFavorites = previews.filter(item => {
		...
		const list = sortedFavorites.map(({ title, id, updated, image, genres, seasons }) => {
		...
		const listHandler = async () => store.loadList();
		const singleHandler = async () => store.loadSingle('8801');

		return html`
		<div>
			<a @click="${ listHandler }">Go to list</a>
...
// @ts-ignore
customElements.define('podcast-view-favorites', Component);